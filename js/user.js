...
function contriData(data,username) {
    ...
    $('.user-detail-contribution').html(aHtml);

    //点击某一个标题进入当前稿件的详情
    $('.contri-title').on('click', function () {
        var _this = $(this);
        $('.user-detail-contribution').addClass('hide');
        $('.user-detail-contribution-person').removeClass('hide');
    ...
    $('.edit-upload-btn').on('click', function () {
        ...
        var fileAccept = $(this).val().split(".")[1];//获取上传文件的后缀
    ...
    //渲染当前稿件详情
    function personContri(data) {
        var bHtml = [], cHtml= [], dHtml=[], eHtml=[];
        bHtml.push('<span class="contri-callback">返回</span> <span class="contribute-upload-single upload-on clearfloat" data-id="'+ data.docu_id+'"><i class="upload-icon"></i>下载原始摘要</span><span class="contribute-upload-all upload-on clearfloat"><i class="upload-icon"></i><em>下载原始全文</em></span>');
        ...
        $('.contri-callback').on('click', function () {
            var _this = $(this);
            $('.user-detail-contribution').removeClass('hide');
            $('.user-detail-contribution-person').addClass('hide');
        });
        //点击稿件下载
        $('.contribute-upload-single').on('click', function () {
            var cid = $(this).attr('data-id');
            console.log(url + '/app/data/'+cid);
            window.open(url + '/app/data/'+cid);
        });
        ...
        success: function (data) {
            if (data.status == 1) {
                if(data.data[0].imgSrc){
                    $('.detail-right-title-img').attr('src',data.data[0].imgSrc);
                }
...
function perInfo(data) {
    ...
    pHtml.push('<div class="user-person-info-list person-info-sex"><label class="label-sex">性别：</label><select class="user-person-info-val val-sex"><option value="0" selected=' + sex + '>男</option><option value="1" selected=' + sex + '>女</option></select></div>');
    ...
    pHtml.push('<div  class="user-person-info-list val-phone"> <label>手机号码：</label> <input type="text" placeholder="请输入11位数字的手机号" class="user-person-info-val val-phone" value="' + data.phone + '"/> </div>');
...
function submission(username) {
    ...
    $('.save-btn-confirm').on('click', function () {
        ...
        if(!/[\u4E00-\u9FA5\uF900-\uFA2D]/.text(chineseTitle)){
            $('.chinese-tit-error').html('请填写中文标题');
            return false
        }
        if(!(/[\u4E00-\u9FA5\uF900-\uFA2D]||\，*||\,*/.text(keyChinese))){
        ...
        if(authorPing==''||/[\u4E00-\u9FA5\uF900-\uFA2D]/.text(authorPing)){
